...
// 打包的文件名
const fileName = (()=>{
    var pt = folderName.split(':')[1] || 'index';
    return pt;
})();
// 监控 || 打包
const isWatch = (()=>{
...
let method = {
    ...
    compileless(){
        var from = path.join(__dirname,projectName,'/',platformName,'./src/less/*.less');
        var to = path.join(__dirname,projectName,'/',platformName,'./dest/css/');
        compile();
        function compile(){
            gulp.src(from) //多个文件以数组形式传入
    ...
    compileimg(){
        ...
        var to = path.join(__dirname,projectName,'/',platformName,'/','./dest/images/');
        compile();
        function compile(){
            gulp.src(from)
            .pipe(cache(imagemin({
    ...
    compilehtml(){
        var from = path.join(__dirname,projectName,'/',platformName,'./*.html');
        var to = path.join(__dirname,projectName,'/',platformName,'/','./dest/');
        compile();
        function compile() {
            gulp.src(from)
                .pipe(htmlmin({collapseWhitespace: true}))
